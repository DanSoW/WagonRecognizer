...
import client.network.InvoiceNetwork;
...
public class Invoices extends Application {
    ...
    public void start(Stage stage) throws Exception {
        ...
        MenuItem regCam = new MenuItem("Зарегистрированные камерой полувагоны");
        MenuItem reg = new MenuItem("Зарегистрированные полувагоны по накладной");

        regCam.setOnAction(new EventHandler<ActionEvent>() {
            @Override
        ...
        _addInvoice.addEventHandler(MouseEvent.MOUSE_CLICKED, new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                if(!DataValidator.requiredValidator(new String[]{
                    ...
                    MessageShow(Alert.AlertType.ERROR, "Ошибка!", "Общее число полувагонов должно быть целым числом!");
                    return;
    ...
    private void readDataInvoices(){
        ...
        try {
            elements = InvoiceNetwork.getListDataInvoices("http://localhost:8080/database/invoices/get/all");
        } catch (Exception e) {
